...
def draw_instructions(surface, mouse_pos):
    ...
    try:
        font = pygame.font.SysFont("malgungothic", 32)  # 글자 키우고
        title_font = pygame.font.SysFont("malgungothic", 60)  # 타이틀 더 크게
    except:
        font = pygame.font.SysFont(None, 32)
        title_font = pygame.font.SysFont(None, 60)

    y = 60
    line_height = 40  # 줄 간격 넉넉하게
    ...
    for name, color, desc in planets_info:
        pygame.draw.circle(surface, color, (circle_x, y + 15), PLANET_RADIUS // 2 + 2)
    ...
    for line in fuel_text:
        ...
        y += line_height + 10  # 줄 간격 조금 더

    # 뒤로가기 버튼
    global back_button_rect
    back_button_rect = pygame.Rect(WIDTH - 140, HEIGHT - 80, 120, 50)
...
while running:
    ...
    if shake_duration > 0:
        shake_offset.x = random.randint(-shake_magnitude, shake_magnitude)
        shake_offset.y = random.randint(-shake_magnitude, shake_magnitude)
        shake_duration -= dt
    ...
    if state == "menu":
        ...
        if not game_over:
            ...
            if ship.check_collision(planets):
                game_over = True
                explosion_timer = 1.5
                shake_duration = 0.5  # 충돌시 0.5초간 흔들림
        ...
        if game_over:
            ...
            final_score_rect = final_score_surf.get_rect(center=(WIDTH // 2, HEIGHT // 2 - 20))
