...
def draw_instructions(surface, mouse_pos):
    try:
        ...
        title_font = pygame.font.SysFont("malgungothic", 56)
    except:
    ...
    back_button_rect = pygame.Rect(WIDTH - 120, HEIGHT - 70, 110, 50)
...
while running:
    ...
    if state == "menu":
        ...
        if game_over and explosion_timer > 0:
            shake_magnitude = 5
            shake_offset = pygame.Vector2(random.uniform(-shake_magnitude, shake_magnitude),
        ...
        if not game_over:
            score_text = pygame.font.render(f"점수: {score:.0f}", True, WHITE)
            fuel_text = pygame.font.render(f"연료: {ship.fuel:.1f}", True, WHITE)
            highscore_text = pygame.font.render(f"최고 기록: {highscore:.0f}", True, WHITE)
            ...
            try:
                font_big = pygame.font.SysFont("malgungothic", 72)
                font_medium = pygame.font.SysFont("malgungothic", 48)
            except:
                font_big = pygame.font.SysFont(None, 72)
                font_medium = pygame.font.SysFont(None, 48)

            game_over_surf = font_big.render("게임 오버", True, WARNING_COLOR)
            score_surf = font_medium.render(f"점수: {score:.0f}", True, WHITE)
            highscore_surf = font_medium.render(f"최고 기록: {highscore:.0f}", True, WHITE)

            screen.blit(game_over_surf, (WIDTH // 2 - game_over_surf.get_width() // 2, HEIGHT // 3))
            screen.blit(score_surf, (WIDTH // 2 - score_surf.get_width() // 2, HEIGHT // 3 + 80))
            screen.blit(highscore_surf, (WIDTH // 2 - highscore_surf.get_width() // 2, HEIGHT // 3 + 140))

            button_rect = pygame.Rect(WIDTH // 2 - 110, HEIGHT // 2 + 110, 220, 60)
