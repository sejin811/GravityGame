...
def draw_menu(surface, mouse_pos):
    ...
    draw_button(surface, start_button_rect, "Start Game", mouse_pos)
    draw_button(surface, instructions_button_rect, "Instructions", mouse_pos)
...
def draw_instructions(surface, mouse_pos):
    ...
    title_surf = title_font.render("Instructions", True, WHITE)
    surface.blit(title_surf, (WIDTH // 2 - title_surf.get_width() // 2, 10))

    # 행성 종류와 색상 표시
    planets_info = [
        ("Red Planet", RED_COLOR, "중력: 강함 (1500)"),
        ("Green Planet", GREEN_COLOR, "중력: 중간 (1000)"),
        ("Blue Planet", BLUE_COLOR, "중력: 약함 (500)"),
    ]
    ...
    draw_button(surface, back_button_rect, "Back", mouse_pos)
...
while running:
    ...
    if state == "menu":
        ...
        score = ship.distance_traveled / 10
        score_text = pygame.font.SysFont(None, 30).render(f"Score: {score:.0f}", True, WHITE)
        fuel_text = pygame.font.SysFont(None, 30).render(f"Fuel: {ship.fuel:.1f}", True, WHITE)
        highscore_text = pygame.font.SysFont(None, 30).render(f"Best: {highscore:.0f}", True, WHITE)
        ...
        if game_over:
            ...
            draw_button(screen, button_rect, "Restart", mouse_pos)
