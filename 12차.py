...
class Spaceship:
    ...
    def draw(self, surface, camera_offset, shake_offset=pygame.Vector2(0,0)):
        draw_pos = self.pos - camera_offset + pygame.Vector2(WIDTH // 2, HEIGHT // 2) + shake_offset
        if self.alive:
...
def draw_warning(surface, ship, planets, camera_offset, shake_offset=pygame.Vector2(0,0)):
    for planet in planets:
        distance = (ship.pos - planet.pos).length()
        if distance < WARNING_DISTANCE + PLANET_RADIUS:
            draw_pos = planet.pos - camera_offset + pygame.Vector2(WIDTH // 2, HEIGHT // 2) + shake_offset
...
def draw_map_boundary_warning(surface, camera_offset, shake_offset=pygame.Vector2(0,0)):
    top_left = pygame.Vector2(-MAP_HALF, -MAP_HALF) - camera_offset + pygame.Vector2(WIDTH // 2, HEIGHT // 2) + shake_offset
...
def reset_game():
    global ship, planets, fuelpods, game_over, explosion_timer, shake_time
    ship = Spaceship(0, 0)
    ...
    if state == "menu":
        ...
        if not game_over:
            ...
            if explosion_timer > 0:
                ...
                if shake_time > 0:
                    shake_time -= dt
                    shake_offset.x = random.uniform(-5, 5)
                    shake_offset.y = random.uniform(-5, 5)
                else:
                    shake_offset = pygame.Vector2(0, 0)
            else:
                shake_offset = pygame.Vector2(0, 0)
                # 게임 오버 상태에서 점수 및 버튼 그리기
                # (아래에서 별도 처리)

        camera_offset = ship.pos

        draw_map_boundary_warning(screen, camera_offset, shake_offset)
        for planet in planets:
            planet.draw(screen, camera_offset, shake_offset)
        for pod in fuelpods:
            pod.draw(screen, camera_offset, shake_offset)

        draw_warning(screen, ship, planets, camera_offset, shake_offset)
        ship.draw(screen, camera_offset, shake_offset)
        ...
        if game_over and explosion_timer <= 0:
            ...
            over_font = None
            try:
                over_font = pygame.font.SysFont("malgungothic", 48)
            except:
                over_font = pygame.font.SysFont(None, 48)
            game_over_text = over_font.render("게임 오버", True, WARNING_COLOR)
            game_over_rect = game_over_text.get_rect(center=(WIDTH // 2, HEIGHT // 2 - 60))
            screen.blit(game_over_text, game_over_rect)

            final_score_text = game_font.render(f"최종 점수: {score:.0f}", True, WHITE)
